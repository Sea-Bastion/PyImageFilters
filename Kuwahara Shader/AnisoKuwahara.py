...
LocalOrientation = np.mean(LocalOrientation, axis=2)
Scalers = np.array([ RawAnisoData['a'], RawAnisoData['b'] ]).transpose(1,2,3,0)
...
def ModCorr(ImageSample, Mask):
    
    dims = ImageSample.shape
    Center = (dims[0], dims[1])//2
    ScaleFactors = [ ImageSample[Center, 3], ImageSample[Center, 4] ]
    Rotation = ImageSample[Center, 5]
...
def ProcessDirection(n):
    ...
    #stack all of the data into one array for the filter function
    ImageDataMatrix = np.concatenate( [Img, Scalers.transpose(1,0,2), LocalOrientation.T[:,:,np.newaxis]], axis=-1)
    ...
    Mean = ndimage.generic_filter(ImageDataMatrix, ModCorr, size=[101,101, 6], extra_arguments=(Masks[n],))
