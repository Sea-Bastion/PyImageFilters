...
with open(MaskPath, 'r') as MaskFile:
    Mask = cp.array(json.load(MaskFile)[0])
...
FlatOutput = cp.zeros(Img.shape, dtype=cp.float32).flatten()
FlatMask = Mask.flatten().astype(cp.float32)

# threadsPerBlock = ( dims[0], )
# BlocksPerGrid = ( dims[1], )
threadsPerBlock = ( 1, )
BlocksPerGrid = ( 1, )
Args = (ImageTexture, cp.array(dims, dtype=cp.uint32), FlatMask, 51, FlatOutput)
